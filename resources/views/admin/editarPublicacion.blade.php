...
<body class="d-flex flex-column min-vh-100">
    ...
    <section id="perfilUsuario">
                        ...
                        <div class="card py-4">
                            <div class="row">
                                <div class="container col-12 col-lg-7 p-5 pt-3 mt-1">
                                    ...
                                    <!-- Formulario -->   
                                    <form class="php-email-form" action="{{ url('/publicacion/' . $publicacion->id) }}" method="POST" enctype="multipart/form-data">
                                        @csrf
                                        {{ method_field('PATCH') }}
                                        <p class="m-2 text-muted px-2">Texto</p>
                                        <div class="form-row p-3 pt-0">
                                            <div class="form-group pb-2">
                                                <input type="text" name="titulo" value="{{ $publicacion->titulo }}" class="form-control text-center" id="titulo" placeholder="Introduzca un título" required>
                                            </div>
                                            <div class="form-group py-2">
                                                <textarea class="form-control text-center" name="contenido" value="{{ $publicacion->contenido }}" rows="3" id="contenido" placeholder="Introduzca un contenido" required></textarea>
                                            </div>
                                            <p class="m-2 text-muted px-2">Imagen</p>
                                            <div class="form-group py-2">
                                                <input type="file" class="form-control" name="imagenPublicacion" value="{{ $publicacion->imagenPublicacion }}" id="imagenPublicacion" required>
                                            </div>
                                        </div>
                                        <div class="btns mt-5 container d-flex justify-content-center">
                                            <input id="registrar" type="submit" class="px-5 mx-2" value="Guardar cambios">
                                        </div>
                                    </form>
                                </div>
                            </div>
        ...
        <div class="btns container d-flex justify-content-center mt-5 mb-5">
            <a href="../" class="btn col-lg-3 px-3">&nbsp;Listado de publicaciones&nbsp;</a>
        </div>
    </section>
    <!-- Footer -->
